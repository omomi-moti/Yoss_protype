 ...
 * 画面Dで支援の方向性Bのどの項目に出したいかから決めている。Bの項目側に領域を足すと
 * 無関係な支援まで引き込むため（③居場所に「経済」を足すと就学援助が出る）、
 * 支援側に領域を足して寄せる。対応表は issue #25 のコメントを参照。
 */
...
const seeds: OrganizationSeed[] = [
  {
    ...
    supports: [
      { id: 'r4', name: '子ども食堂', categories: ['地域情報', '経済'], description: '無料または低額の食事提供と居場所', targetGrades: '全学年', cost: '無料〜100円', capacity: '各30名程度', frequency: '週2回（火・金 17:00〜19:00）', howToUse: '学校から電話1本で受付。初回は職員が同席して顔つなぎをします', enabled: true },
      { id: 's-pantry', name: 'フードパントリー', categories: ['経済', '地域情報'], description: '食料品の無償配布（月2回）', targetGrades: '-', cost: '無料', capacity: '月40世帯', frequency: '月2回（第2・第4土曜）', howToUse: '事前申込制。学校経由の紹介も受け付けます', enabled: true },
    ],
    reviews: [
      {
        schoolName: '○○小学校',
  ...
  },
  {
    ...
    supports: [
      { id: 'r7', name: '学習支援教室', categories: ['学習'], description: '大学生ボランティアによる学習支援', targetGrades: '小4〜中3', cost: '無料', capacity: '各回15名', frequency: '週1回（水 16:00〜18:00）', howToUse: '空き枠を電話で確認のうえ申込。体験参加も可', enabled: true },
      { id: 'r5', name: '放課後子ども教室', categories: ['地域情報'], description: '放課後の安全な居場所と体験活動', targetGrades: '小1〜小6', cost: '無料', capacity: '各校40名', frequency: '平日毎日（放課後〜17:30）', howToUse: '各校の窓口で年度当初に登録。途中参加も可', enabled: true },
    ],
    reviews: [
      {
        schoolName: '△△小学校',
  ...
  },
  {
    ...
    supports: [
      { id: 'r9', name: '保護者相談窓口', categories: ['家庭状況'], description: '子育ての悩み全般に対応（LINE相談は24時間受付）', targetGrades: '-', cost: '無料', capacity: '予約制', frequency: '平日 9:00〜17:00（LINEは24時間受付）', howToUse: '保護者から直接、または学校からの取次ぎでも可', enabled: true },
      { id: 'r10', name: '家庭訪問支援', categories: ['家庭状況', '福祉'], description: '養育支援訪問事業', targetGrades: '-', cost: '無料', capacity: '月10件程度', frequency: '月2〜4回（1回2時間程度）', howToUse: '学校またはSSWから申込。事前に家庭との面談があります', enabled: true },
      { id: 's-ycarer', name: 'ヤングケアラー相談', categories: ['福祉', '家庭状況'], description: '専用相談窓口・ヘルパー派遣制度の案内', targetGrades: '全学年', cost: '無料', capacity: '随時', frequency: '随時（要予約）', howToUse: '学校から連絡。本人・保護者の同意を得たうえで面談します', enabled: true },
      { id: 'r11', name: '保護者プログラム', categories: ['家庭状況'], description: 'ペアレントトレーニング（全6回）', targetGrades: '-', cost: '無料', capacity: '各回10名', frequency: '全6回（隔週・土曜午前）', howToUse: '年2期の募集時に申込。全回参加が原則です', enabled: false },
    ],
    reviews: [
      {
        schoolName: '□□中学校',
  ...
  },
  {
    ...
    supports: [
      { id: 'r6', name: 'フリースクール通所', categories: ['学校適応', '学習', '地域情報'], description: '不登校児童生徒の学びの場（出席認定対応）', targetGrades: '小1〜中3', cost: '月額5,000円〜', capacity: '15名', frequency: '週1〜5日（日数は選択制）', howToUse: 'まず見学だけの相談も歓迎。面談後、在籍校と出席認定を確認します', enabled: true },
      { id: 's-online', name: 'オンライン学習サポート', categories: ['学習'], description: '通所が難しい子ども向けの在宅学習支援', targetGrades: '小4〜中3', cost: '月額3,000円', capacity: '10名', frequency: '週2回（各60分）', howToUse: 'Webから申込。端末の貸出あり', enabled: false },
    ],
    reviews: [
      {
        schoolName: '○○小学校',
  ...
  },
  {
    ...
    supports: [
      { id: 'r1', name: '就学援助', categories: ['経済'], description: '学用品費・給食費等の援助（オンライン申請可）', targetGrades: '小1〜中3', cost: '無料', capacity: '所得基準あり', frequency: '年1回（年度途中の申請も可）', howToUse: 'オンライン申請。学校向けに案内文書のひな形を提供しています', enabled: true },
      { id: 'r14', name: '適応指導教室', categories: ['学校適応'], description: '不登校児童生徒の学校復帰を支援', targetGrades: '小1〜中3', cost: '無料', capacity: '各20名', frequency: '平日毎日（9:00〜14:00）', howToUse: '在籍校を通じて申込。事前に教育相談を受けます', enabled: true },
      { id: 'r13', name: 'スクールカウンセラー連携', categories: ['福祉', '学校適応'], description: 'SC・SSWとの連携による心理／福祉支援', targetGrades: '全学年', cost: '無料', capacity: '各校週1回', frequency: '各校 週1回の巡回', howToUse: '学校から派遣を依頼。緊急時は別途相談', enabled: true },
      { id: 'r3', name: '給付型奨学金', categories: ['経済'], description: '返済不要の奨学金', targetGrades: '中3', cost: '無料', capacity: '年間20名', frequency: '年1回（12月募集）', howToUse: '学校長の推薦を得たうえで申請', enabled: false },
    ],
    reviews: [
      {
        schoolName: '○○小学校',
  ...
  },
  {
    ...
    supports: [
      { id: 'r12', name: '発達相談', categories: ['発達'], description: '発達に関する専門相談（医師同席枠あり）', targetGrades: '全学年', cost: '無料', capacity: '月12件', frequency: '月2回（専門医が同席する枠あり）', howToUse: '電話予約。待機は2週間程度。学校からの情報提供書があると初回が円滑です', enabled: true },
      { id: 'r2', name: '医療費助成', categories: ['健康'], description: '18歳以下の医療費自己負担分を助成', targetGrades: '全学年', cost: '無料', capacity: '制限なし', frequency: '通年', howToUse: '市の窓口で受給者証を申請', enabled: true },
    ],
    reviews: [],
    isMine: false,
  },
  {
    ...
    supports: [
      { id: 's-seat', name: '無償学習席の提供', categories: ['学習'], description: '学習塾の空き枠を無償開放（学校推薦制）', targetGrades: '小5〜中3', cost: '無料', capacity: '各教室5名', frequency: '週3回（月・水・金 17:00〜21:00）', howToUse: '学校またはSSWの推薦が必要。教材費も含めて費用はかかりません', enabled: true },
    ],
    reviews: [],
    isMine: false,
  },
