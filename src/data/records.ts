...
export function entriesByRole(student: Student): { role: StaffRole; entries: ScreeningEntry[] }[] {
  const entries = student.problems.map(tag => {
    const { role, item, note, judgement } = SCREENING_ITEMS[tag];
    return { role, item, note, judgement };
  });
...
export function screeningRows(
  student: Student,
  role: StaffRole
): { item: string; hasInput: boolean; judgement: '経過観察' | '要注意' | null }[] {
  const studentTags = new Set(student.problems);

  return (Object.keys(SCREENING_ITEMS) as ProblemTag[])
    .filter(tag => SCREENING_ITEMS[tag].role === role)
    .map(tag => ({
      item: SCREENING_ITEMS[tag].item,
      hasInput: studentTags.has(tag),
      judgement: studentTags.has(tag) ? SCREENING_ITEMS[tag].judgement : null,
    }));
}
