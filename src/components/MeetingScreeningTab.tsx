...
import ScreeningDataPanel from './ScreeningDataPanel';
import ScreeningDirectionPanel from './ScreeningDirectionPanel';
import type { StaffRole, Student } from '../types';

/** 実物のスクリーニング画面のサブタブ。担当別の入力はひとまとめにしている */
type ScreeningView = 'direction' | 'data' | 'role';

const VIEWS: { key: ScreeningView; label: string }[] = [
  { key: 'direction', label: '支援の現状' },
  { key: 'data', label: 'データ' },
  { key: 'role', label: '担当別の入力' },
];

/**
 * タブ②「スクリーニング／会議記録」。
 *
 * 実物のスクリーニング画面と同じサブタブ構成にしている。
 *   支援の現状 … 方向性 A/B/C ごとに、実際に何をしているかを 新／続／拒 で記録する面
 *   データ　　 … スクリーニング37項目。この合計が領域スコアになる
 *   担当別の入力 … 担当ごとの項目と AI判定
 *
 * 会議での決定は右ドロワーとタブ⑤に一本化しているので、ここには決定の入力を置かない。
 * 同じ決定を2箇所で持つと食い違うため。
 */
export default function MeetingScreeningTab({
  ...
  const [view, setView] = useState<ScreeningView>('direction');
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-4">
      <div className="flex flex-col gap-3 mb-4">
        ...
        {view === 'data' && <ScreeningDataPanel student={student} />}
          ...
          <div className="grid grid-cols-[1.9fr_1fr] gap-5 items-start">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">
                ...
                会議で決めたことは右の「この会議の記録をつける」に記録します。
              </p>
            </div>
